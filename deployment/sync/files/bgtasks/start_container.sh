...
files=$(ls /home/wercker/iids/ -l | sed -rn 's/.*(bgtasks_.+)/\1/gp')
for file in $files ; do
	if ! [ -a /home/wercker/cids/$file ] ; then
		...
		sudo docker run -d --name $file --cidfile /home/wercker/cids/$file --link $mongo_name:mongo --link $postgres_name:postgres demokratikollen/bgtasks:$file
	fi
done